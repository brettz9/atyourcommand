								...
								createProcessAtPath(
									...
									data.args.map(function (argVal) {
										...
										return argVal.replace(
											/<(save-temp-)?(uc?encode-)?(?:(pageTitle)|(pageURL)|(pageHTML)|(selectedHTML)|(selectedText)|(bodyText)|(linkLocation)|(imageLocation))>/,
											function () {
												// ucencode needs encodeURIComponent applied
												// add link content option also (retrieve content of referenced link here since not yet available)
