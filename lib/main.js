...
function picker (data, emit) {
    ...
    fp.init(
        ...
        selectFolder ? "Pick a folder for the executable" : "Pick an executable file", // Todo: Fix make this dynamic and update across add-ons
    ...
    onMessage: function (e) {
        ...
        case 'one-off':
            ...
            win.addEventListener('load', function () {
                tabs.activeTab.on('ready', function (tab) {
                    var worker = tab.attach({
                        ...
                        contentScriptOptions: { // any JSON-serializable key/values
							...
							//,locale // Todo: Pass locale data here for use by ExpandableInputs.js (parse properties file to get these?)
                        }
                    });
                    var on = worker.port.on,
                        emit = worker.port.emit;
                ...
                tabs.activeTab.url = data.url('one-off.html'); // Todo: Pass locale within HTML file name or use "data-l10n-id" attribute in HTML (ugly but more maintenable with HTML changes); better yet, just replace HTML with Jamilih
            });
            break;
        case 'prefs':
