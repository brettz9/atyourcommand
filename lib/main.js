...
cm.Menu({
	...
	onMessage: function (e) {
		...
		case 'commands': case 'one-off':
			// Todo: Save coordinates on close and modify openDialog to accept these for initial dimensions
			win = openDialog({
				name: _("One-off"),
					...
					on({
						...
						buttonClick: function (data) {
							if (data.remove) {
								...
								emit('removeStorage', ss.commands);
							}
							if (data.storage) {
								// Todo: Utilize saving by user and prevent unintentional overwriting
								save(data.name, data.storage);
								emit('newStorage', ss.commands);
							}
							if (data.execute) {
								/*
								createProcessAtPath(
