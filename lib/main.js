...
function execute (name) {
	var data = ss.commands[name].storage;
	/*
	createProcessAtPath(
...
function addDynamicCMContent (name, data) {
	...
	var item = dynamicCMItems[name] = cm.Item({
		label: name,
		context: 
		contentScript: , // contentScriptFile will load before this if we want some default code
