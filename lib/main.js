...
cm.Menu({
	...
	onMessage: function (clickData) {
		...
		case 'one-off':
			...
			panel.show();
			panel.port.on('autocompleteValues', function (data) {
				panel.port.emit('autocompleteValuesResponse', autocompleteValues(data));
			});
			panel.port.on('filePick', function (data) {
				picker(data, function (arg1, arg2) {
					panel.show();
					panel.port.emit(arg1, arg2);
				});
				panel.port.emit('filePickResponse');
			});
