...
var mainMenu, dynamicCMItems = {}, dynamicCMItems2 = {};
function addDynamicCMContent (name, details) {
	// todo: handle details.ownContext
	// todo: handle details.restrictContexts
	var itemConfig = {
		label: name,
		data: {selector: 'dynamic_' + name, property: ''}, // Namespace to distinguish from our built-in names
	...
	var item = dynamicCMItems[name] = cm.Item(itemConfig);
	
	// We need to duplicate contexts since array of contexts not working properly
	itemConfigCopy = Object.create(itemConfig);
	itemConfigCopy.context = SelectionContext();
	var item = dynamicCMItems2[name] = cm.Item(itemConfigCopy);
